...
import { getRow, getCol, startingpointStyle, squareStyle  } from "./models/BoardSettings";
...
import { getStartingPosition, getStartingCase, setBoardSettings, setDesiredSquare } from "./boardSlicer";
    ...
    await dispatch(fetchParticipant()).then(async () => {
      await dispatch(fetchClubs()).then(async () => {
        ...
        await dispatch(getStartingPosition()).then(async () => {
          await dispatch(getStartingCase())
          .then(async ()=>{
